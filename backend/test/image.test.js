...
test('preset: ค่าคงที่ตรงตามดีไซน์ (OCR คมชัด/ใหญ่กว่า, STORAGE เล็ก/ประหยัด)', () => {
  assert.deepEqual(OCR_PRESET, { maxDim: 1536, quality: 92 })
...
test('compressImage: รูปใหญ่ถูกย่อให้ด้านยาวสุด ≤ maxDim + แปลงเป็น JPEG', async () => {
  ...
  assert.equal(meta.width, 1536, '3000x2000 → ย่อตามด้านกว้างเป็น 1536')
  assert.equal(meta.height, 1024, 'รักษาสัดส่วน (1536/3000*2000 = 1024)')
})
